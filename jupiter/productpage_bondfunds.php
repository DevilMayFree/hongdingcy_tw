...
<link rel='canonical' href='https://www.cgsice.com/jupiter/productpage_bondfunds.php' />
<link rel='shortlink' href='' />
	<title>Jupiter先機債券型基金</title>
  <meta name='title' content='Jupiter先機債券型基金' />
	<meta name='description' content='' />
	<meta name='keywords' content='' />
	<meta name='sitecode' content='zh-TW'> <!--動態產生-->

	<!--Line-->
	<meta itemprop='name' content='Jupiter先機債券型基金'>
	<meta itemprop='url' content='https://www.cgsice.com/jupiter/productpage_bondfunds.php'>
	...
	<!--FB-->
	<meta property='og:title' content='Jupiter先機債券型基金'>
	<meta property='og:type' content='website'>	
	<meta property='og:url' content='https://www.cgsice.com/jupiter/productpage_bondfunds.php'>
...
<script src="js/main.js" defer></script>    <link rel="stylesheet" href="css/funds.css?ver=1639710965">
    <script type="text/javascript">
    $(function(){
    	$("body").on("click",".page_tabs .tab",function(){
    		var banner_title=$(this).find("h5").html();
    		var pic=$(this).data("pic");
    		$("#banner_title").html(banner_title);
    		$("#search_title").html(banner_title+"搜尋");
    		$("#banner").css("background-image","url("+pic+")");
    		$(".page_tabs .tab").removeClass("active");
    		$(this).addClass("active");
    		var type=$(this).data("type");
    		$("#type").val(type);
    		$.post("get_fund_form.php",{type:type},function(data){
    			var d=JSON.parse(data);
        	$("#risk_level option").remove();
        	// $("#risk_level").append($("<option></option>").attr("value","").text("請選擇"));
        	$.each(d["risk_level"],function(k,title){
       			$("#risk_level").append($("<option></option>").attr("value",title).text(title));
       		})
       		$("#risk_level").multipleSelect("refresh");//更新已變更的下拉選單
       		$("#risk_level_select_amount").html(0);
       		$("#risk_level_total_amount").html($("#risk_level option").length);

       		$("#share_type option").remove();
        	// $("#share_type").append($("<option></option>").attr("value","").text("請選擇"));
        	$.each(d["share_type"],function(k,title){
       			$("#share_type").append($("<option></option>").attr("value",title).text(title));
       		})
       		$("#share_type").multipleSelect("refresh");//更新已變更的下拉選單
       		$("#share_type_select_amount").html(0);
       		$("#share_type_total_amount").html($("#share_type option").length);
       		
       		$("#invest_area option").remove();
        	// $("#invest_area").append($("<option></option>").attr("value","").text("請選擇"));
        	$.each(d["invest_area"],function(k,title){
       			$("#invest_area").append($("<option></option>").attr("value",title).text(title));
       		})
       		$("#invest_area").multipleSelect("refresh");//更新已變更的下拉選單
       		$("#invest_area_select_amount").html(0);
       		$("#invest_area_total_amount").html($("#invest_area option").length);

       		$("#interest_type option").remove();
        	// $("#interest_type").append($("<option></option>").attr("value","").text("請選擇"));
        	$.each(d["interest_type"],function(k,title){
       			$("#interest_type").append($("<option></option>").attr("value",title).text(title));
       		})
       		$("#interest_type").multipleSelect("refresh");//更新已變更的下拉選單
       		$("#interest_type_select_amount").html(0);
       		$("#interest_type_total_amount").html($("#interest_type option").length);

       		$("#currency option").remove();
        	// $("#currency").append($("<option></option>").attr("value","").text("請選擇"));
        	$.each(d["currency"],function(k,title){
       			$("#currency").append($("<option></option>").attr("value",title).text(title));
       		})
       		$("#currency").multipleSelect("refresh");//更新已變更的下拉選單
       		$("#currency_select_amount").html(0);
       		$("#currency_total_amount").html($("#currency option").length);
       		$("#send_btn").trigger("click");
    		})
    	})
    	$("#clear_data_btn").click(function(){
    		$("#risk_level").multipleSelect("uncheckAll");
    		$('#share_type').multipleSelect("uncheckAll");
    		$('#invest_area').multipleSelect("uncheckAll");
    		$('#interest_type').multipleSelect("uncheckAll");
    		$('#currency').multipleSelect("uncheckAll");
    	})
    	$("select").change(function(){
    		var my_btn=$(this);
    		var id=my_btn.attr("id");
    		var amount=$("#"+id+" :selected").length;
    		$("#"+id+"_select_amount").html(amount);
    	})
    })
    </script>
</head>
<body>
...
</div>    <div id="banner" style="background-image: url(img/funds/01_banner.jpg);">
    ...
    <div class="inner w1200 txt-white">
        <div class="text">
            <h2 class="h1 txt-bold" id="banner_title">債券型基金</h2>
            <h5></h5>
        </div>
    </div>
</div>    <div class="page_tabs">
                    <a href="productpage_equityfunds.php" data-type="22"  class="tab " data-pic="https://www.cgsice.com/jupiter/img/funds/01_banner2.jpg">
                <h5 class="txt-bold txt-theme">股票型基金</h5>
            </a>
                      <a href="productpage_bondfunds.php" data-type="21"  class="tab active" data-pic="https://www.cgsice.com/jupiter/img/funds/01_banner.jpg">
        ...
        <div class="w1200">
            <form class="inner" id="form1" name="form1">
                <div class="col col100">
                    <p class="txt-bold txt-orange" id="search_title">
                        債券型基金搜尋
                    </p>
                    <input type="text" id="kw" name="kw" value="" placeholder="以基金名稱搜尋">
                </div>
                <div class="col col33">
                    <p class="txt-bold txt-orange flex-sb">
                        風險等級
                        <span class="small txt-medium txt-dark">
                            <span id="risk_level_select_amount">0</span>/<span id="risk_level_total_amount">4</span>
                        </span>
                    </p>
                    <select name="risk_level[]" id="risk_level" multiple="multiple" class="multiple-select" placeholder="請選擇">
                    	                        <option value="RR4">RR4</option>
                    		                        <option value="RR5">RR5</option>
                    		                        <option value="RR3">RR3</option>
                    		                        <option value="RR2">RR2</option>
                ...
                <div class="col col33">
                    <p class="txt-bold txt-orange flex-sb">
                        股份類別
                        <span class="small txt-medium txt-dark">
                            <span id="share_type_select_amount">0</span>/<span id="share_type_total_amount">12</span>
                        </span>
                    </p>
                    <select name="share_type[]" id="share_type" multiple="multiple" class="multiple-select" placeholder="請選擇">
                    	                    		<option value="L類累積股">L類累積股</option>
                    		                    		<option value="B類累積股">B類累積股</option>
                    		                    		<option value="C類累積股">C類累積股</option>
                    		                    		<option value="C2類累積股">C2類累積股</option>
                ...
                <div class="col col33">
                    <p class="txt-bold txt-orange flex-sb">
                        投資區域
                        <span class="small txt-medium txt-dark">
                            <span id="invest_area_select_amount">0</span>/<span id="invest_area_total_amount">5</span>
                        </span>
                    </p>
                    <select name="invest_area[]" id="invest_area" multiple="multiple" class="multiple-select" placeholder="請選擇">
                    	                    		<option value="美國">美國</option>
                    		                    		<option value="新興市場">新興市場</option>
                    		                    		<option value="全球">全球</option>
                    		                    		<option value="中國大陸及香港">中國大陸及香港</option>
                ...
                <div class="col col33">
                    <p class="txt-bold txt-orange flex-sb">
                        配息方式
                        <span class="small txt-medium txt-dark">
                            <span id="interest_type_select_amount">0</span>/<span id="interest_type_total_amount">3</span>
                        </span>
                    </p>
                    <select name="interest_type[]" id="interest_type" multiple="multiple" class="multiple-select" placeholder="請選擇">
                    	                    		<option value="年配息">年配息</option>
                    		                    		<option value="不配息">不配息</option>
                    		                    		<option value="月配息">月配息</option>
                    		                    </select>
                ...
                <div class="col col33">
                    <p class="txt-bold txt-orange flex-sb">
                        計價幣別
                        <span class="small txt-medium txt-dark" >
                            <span id="currency_select_amount">0</span>/<span id="currency_total_amount">6</span>
                        </span>
                    </p>
                    <select name="currency[]" id="currency" multiple="multiple" class="multiple-select" placeholder="請選擇">
                    	                    		<option value="美元">美元</option>
                    		                    		<option value="歐元">歐元</option>
                    		                    		<option value="美元穩定配息">美元穩定配息</option>
                    		                    		<option value="南非幣穩定配息">南非幣穩定配息</option>
                ...
                <div class="col col33 btnset txt-right">
                    <p style="opacity: 0;">.</p>
                    <div>
                        <button class="clear" type="button" id="clear_data_btn">
                            <p>
                                清除篩選
                            </p>
                            <img src="img/public/close.svg" alt="">
                        </button>
                        <button class="btn txt-white txt-bold h6" id="send_btn" type="button">
                            搜尋
                        </button>
                    </div>
                </div>
                <input type="hidden" name="type" id="type" value="21">
                <input type="hidden" name="page" id="page" value="1">
                <input type="hidden" name="more_btn" id="more_btn" value="">
            </form>
        </div>
    </div>

    <section id="funds" class="main_padding">
        <div class="inner w1200">
            <div class="itembox" id="show_list">
                
            </div>
            <div class="txt-center">
                <button class="btn-bor" id="get_more" style="display:none" type="button">
                    了解更多
    ...
    <div class="content w1400">
        ...
        <div class="content_bottom">
            <div class="item">
                ...
                <p>客服信箱：<a href="/cdn-cgi/l/email-protection#483b2d3a3e212b2d082b2f3b212b2d662b2725" class="txt-orange"><span class="__cf_email__" data-cfemail="8af9eff8fce3e9efcae9edf9e3e9efa4e9e5e7">[email&#160;protected]</span></a></p>
...
<script>
	$('#risk_level').multipleSelect();
	$('#share_type').multipleSelect();
	$('#invest_area').multipleSelect();
	$('#interest_type').multipleSelect();
	$('#currency').multipleSelect();
	$("body").on("click","#send_btn",function(){
		var my_btn=$(this);
		var more_btn=$("#more_btn").val();
		if(more_btn!="Y")
		{
			$("#page").val(1);
		}
		...
		$.post("get_fund.php",data,function(data){
			d=JSON.parse(data);
			if(d["content"])
			{
				if(more_btn=="Y")
				{
					$("#show_list").append(d["content"]);
				}
				else
				{
					$("#show_list").html(d["content"]);
				}
				$("#more_btn").val("");
			}
			else
